...
- (void)loginWithSdkAppId:(int)sdkAppId
    ...
    if ([self.imManager getLoginStatus] ==  1) {
      self.isLogin = YES;
      self.selfUserId = [self.imManager getLoginUser];
      if (callback) {
...
- (NSString *)sendInvitation:(NSString *)cmd userId:(NSString *)userId content:(NSString *)content callback:(TXCallback)callback {
    ...
    return [self.imManager invite:userId data:jsonString onlineUserOnly:true offlinePushInfo:nil timeout:0 succ:^{
